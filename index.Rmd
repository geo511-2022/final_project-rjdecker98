...
Third, we are going to create a county map of NYS showing the diagnosed diabetes percentage of 2018.
```{r,message=FALSE, warning=FALSE}
...
Now we are going produce a similar map of NYS for the physical inactivity levels from 2018. 
```{r,message=FALSE, warning=FALSE}
NY_Inactivity<-plot_usmap(data = inactivity_2018, values = "Physical.Inactivity.Percentage",include = c("NY"), color = "red") +
   scale_fill_continuous(
    low = "white", high = "red", name = "Physcial Inactivity Percentage", label = scales::comma)+
  labs(title = "Physcial Inactivity Percentage In NY 2018",)+
    theme(legend.position = "bottom")
NY_diabetes
```
Now here is a comparison of the maps side by side. 
```{r,message=FALSE, warning=FALSE}
